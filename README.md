...
### Хранилище Pinia
Все данные приложения централизованно хранятся в модуле gallery.ts.
Pinia обеспечивает управление пагинацией, поиском, и справочниками авторов/локаций, делая данные централизованными и реактивными.
#### Структура состояния (state)
* paintings — список загруженных картин (результаты запроса).
* currentPage — текущая страница пагинации.
* totalCount — общее количество картин (из заголовка ответа API).
* authorsDictionary — справочник авторов (id → имя).
* locationsDictionary — справочник локаций (id → название).
* query — параметры поиска:
* paintingName — подстрока для поиска картин по названию.
* limit — число элементов на одной странице (по умолчанию 6).
#### Действия (actions)
* setPage(page: number) — установить текущую страницу (если значение валидное).
* setTotalCount(total: number) — обновить общее количество картин.
* setAuthors(authors: Author[]) — записать справочник авторов в state.
* setLocations(locations: Location[]) — записать справочник локаций в state.
* setQuery(name: string) — задать строку поиска по названию картины (сброс страницы на первую).
* fetchPaintings() — асинхронная загрузка картин с сервера:
  * отправляется запрос к API /paintings с параметрами _limit, _page, q;
  * обновляются paintings и totalCount на основе ответа сервера.
#### Использование в компонентах
Для работы с хранилищем подключается хук:
```ts
